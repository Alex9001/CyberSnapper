...
Capture profiles accept `colorScheme` (`light`, `dark`, or `both`; default `light`). Both renders each theme in a fresh context with its browser preference set before navigation. If the rendered PNG bytes are identical, redundant dark files and portfolio copies are omitted. This avoids duplicate output and encoding/rendering work, not the second browser load. PDF jobs, unsuccessful or reused/skipped light outputs, and existing dark comparison baselines retain both themes. `target_theme_redundant` reports `omittedArtifacts`; subsequent `job_progress.totalArtifacts` is reduced accordingly. Omitted files are not recorded as completed artifacts.
