...
test('presentation planning never crops, upscales, or exceeds image safety limits', () => {
  for (const aspect of ['auto', '16:9', '4:3', 'square']) {
    const plan = planPresentation(24000, 12000, { ...base, aspect }, { mobile: false }, 'viewport');
  ...
  const widescreen = planPresentation(640, 900, { ...base, aspect: '16:9' }, { mobile: false }, 'viewport');
  assert.ok(Math.abs(widescreen.canvasWidth / widescreen.canvasHeight - 16 / 9) < 0.002);
});

test('auto framing follows capture intent', () => {
  assert.equal(planPresentation(320, 600, base, { mobile: true }, 'viewport').resolvedFrame, 'darkPhone');
  assert.equal(planPresentation(1200, 800, base, { mobile: false }, 'viewport').resolvedFrame, 'lightBrowser');
  assert.equal(planPresentation(1200, 3000, base, { mobile: false }, 'fullPage').resolvedFrame, 'roundedCard');
  assert.equal(planPresentation(600, 400, base, { mobile: false }, 'element').resolvedFrame, 'roundedCard');
});
...
test('every built-in scene and frame renders a valid deterministic canvas', async () => {
  ...
  const frames = ['auto', 'none', 'roundedCard', 'lightBrowser', 'darkBrowser', 'lightPhone', 'darkPhone'];
  ...
  const left = await renderPresentation(source, base, { mobile: false }, 'viewport');
  const right = await renderPresentation(source, base, { mobile: false }, 'viewport');
